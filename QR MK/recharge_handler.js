// ============================================
// RECHARGE HANDLER - Processa recargas via extensão
// ============================================

console.log('[RECHARGE] Handler de recarga carregado');

// Verificar periodicamente por solicitações de recarga
...
function startRechargeMonitoring() {
    ...
    checkInterval = setInterval(async () => {
        try {
            // Buscar solicitação de recarga do backend
            const response = await fetch('http://127.0.0.1:8788/api/get_pending_recharge');
            
            if (!response.ok) {
                if (response.status === 404) {
                    // Sem pendências - isso é normal
                    return;
                }
                console.warn('[RECHARGE] Erro ao buscar pendência:', response.status, response.statusText);
                return;
            }
...
async function processRecharge(uid, key, amount, requestId, returnResultOnly = false) {
    ...
    console.log('[RECHARGE] 💡 O token será buscado de:');
    console.log('[RECHARGE]    1. localStorage (icecassino_token) - PREFERIDO');
    console.log('[RECHARGE]    2. Se não encontrado, será gerado via MD5 - ⚠️ PODE FALHAR');
    
    try {
        // Token agora é assinatura MD5 (gerado abaixo)
        ...
        // ✅ CONSTRUIR PAYLOAD PARA RECARGAS AUTOMÁTICAS
        // Usar valores do template se disponível, senão usar valores apropriados para recargas
        const formPayload = {};
        
        // Parâmetros obrigatórios (sempre presentes)
        formPayload.uid = uid;
        formPayload.key = key;
        ...
        // ✅ CRITICAL: Calculate and add the sign field to the payload
        let signAlgo = null;
        let signSecret = null;
        ...
        // Preferir token capturado da página (token real usado no header)
        let token = null;
        let storageDiags = null;
        try {
            // Primeiro, verificar TUDO no storage
            const allStorage = await chrome.storage.local.get(null);
            console.log('[RECHARGE] 🔍 TODO o storage:', allStorage);
            console.log('[RECHARGE] 🔍 Keys disponíveis:', Object.keys(allStorage));
            
            const t = await chrome.storage.local.get(['icecassino_token']);
            storageDiags = t;
            console.log('[RECHARGE] 🔍 Verificando token em storage:', t);
            if (t.icecassino_token) {
                console.log('[RECHARGE] 🔍 Token encontrado:', t.icecassino_token);
                if (/^[0-9a-f]{32}$/i.test(t.icecassino_token)) {
                    token = t.icecassino_token;
                    console.log('[RECHARGE] ✅ Usando token capturado da página:', token);
                } else {
                    console.log('[RECHARGE] ⚠️ Token em storage não passa na regex:', t.icecassino_token);
                }
            } else {
                console.log('[RECHARGE] ⚠️ Nenhum token em storage');
                console.log('[RECHARGE] 💡 Para capturar o token: faça uma recarga MANUAL no site Ice Casino');
                console.log('[RECHARGE] 💡 O token será automaticamente armazenado e usado para recargas posteriores');
            }
        } catch (e) {
            console.log('[RECHARGE] ❌ Erro ao acessar storage:', e.message);
        }

        if (!token) {
            // Gerar token (assinatura MD5) como fallback - MAS ISSO PODE NÃO FUNCIONAR
            // O ideal é capturar o token REAL que a página usa
            console.log('[RECHARGE] ⚠️ AVISO: Nenhum token capturado! Tentando gerar via MD5 (pode falhar)');
            console.log('[RECHARGE] 💡 DICA: Recarregue a página do cassino e faça uma recarga manual para capturar o token REAL');
            ...
            console.log('[RECHARGE] 📊 Parâmetros para MD5:', signParams);
            console.log('[RECHARGE] 📝 Body string para MD5:', bodyStr);
            ...
            if (!token) {
                console.error('[RECHARGE] ❌ Falha ao gerar token MD5');
                await notifyBackend(requestId, 'error', 'Falha ao gerar token');
                return;
            }
            console.log('[RECHARGE] ⚠️ Token MD5 gerado como fallback:', token.substring(0, 16) + '...');
        }
        
        console.log('[RECHARGE] Enviando requisição...');

        // Tentar via MAIN world (usa axios/interceptors da página)
        const mainPayload = Object.fromEntries(formData.entries());
        
        // DEBUG: Verificar se há diferenças
        console.log('[RECHARGE] 🔍 DIAGNÓSTICO DO PAYLOAD:');
        ...
        // 🔴 DEBUG COMPLETO DO PAYLOAD QUE SERÁ ENVIADO
        console.log('[RECHARGE] 📋 PAYLOAD COMPLETO PARA ENVIO:');
        console.log('═'.repeat(80));
        console.table(mainPayload);  // mainPayload já é um objeto, não precisa converter
        console.log('═'.repeat(80));
        console.log('[RECHARGE] 📝 PAYLOAD STRING:', new URLSearchParams(mainPayload).toString());
        console.log('[RECHARGE] 📏 PAYLOAD LENGTH:', new URLSearchParams(mainPayload).toString().length);
        
        // Enviar payload debug completo
        try {
            chrome.runtime.sendMessage({
                type: 'DEBUG_PAYLOAD_RECHARGE',
                data: mainPayload  // Já é um objeto
            });
        } catch (e) {
            console.log('[RECHARGE] ⚠️ Não conseguiu enviar payload debug:', e.message);
        }
        ...
        if (isSuccess) {
            console.log('%c[RECHARGE] ✅ Recarga realizada com sucesso!', 
            ...
            console.error('[RECHARGE] ❌ Erro na recarga:', result);
            if (!returnResultOnly) {
        ...
        console.error('[RECHARGE] ❌ Exceção ao processar recarga:', error);
        if (!returnResultOnly) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        ...
        (async () => {
            try {
                ...
                if (!amountCentavos || amountCentavos <= 0) {
                    sendResponse({ success: false, error: 'Valor inválido' });
                    return;
                }
                ...
                if (!uid || !key) {
                    sendResponse({ success: false, error: 'Credenciais não encontradas no Ice Cassino' });
                    return;
                }
    ...
    if (message.type === 'CHECK_STORAGE') {
        console.log('[RECHARGE] 🔍 Solicitação de verificação de storage recebida');
        
        chrome.storage.local.get(['icecassino_token', 'icecassino_token_source', 'icecassino_token_time'], (result) => {
            console.log('[RECHARGE] 📊 Storage atual:', result);
...
console.log('[RECHARGE] ✅ Sistema de recarga ativo!');
